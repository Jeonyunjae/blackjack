...
import threadMessages.GameReadyMessage;
...
public class GameClient extends Thread 
{
	...
	public void run() 
	{
		...
		while(!endGame) 
		{
			try {
				...
				if(!readyGame) { 
					if(o instanceof GameReadyMessage) {
						GameReadyMessage m = (GameReadyMessage)o;
						readyGame = m.getReadyFlag();
						return;
					}
				}
				...
				{
					System.out.println("Enter either '1' or 'stand' to stand. Enter either '2' or 'hit' to hit.");
					String choice;
					Scanner in = new Scanner(System.in);
					while(true)
					{
	...
	public static void main(String [] args)
	{
		...
		while(!validFields)
		{
			...
			try 
			{
				gc = new GameClient(ipAddress, 8888);
				validFields = true;
			} catch (IOException ioe) {
	...
	public void startGame()
	{
		...
		/* 1) Ask the user to choose the number of players in the game. This will be a number between 1 and 6, inclusively. */
		/*while(numPlayers < 1 || numPlayers > 3)
		{
			System.out.println("Please choose the number of players in the game (1-6)");
