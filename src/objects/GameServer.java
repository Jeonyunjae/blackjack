...
public class GameServer 
{
	...
	public void checkIfCanStart(String playerName, String gameName, PlayerThread playerThread)
	{
		...
		String startMsgString = "Now start game!";
	...
	public void startRound(String gameName)
	{
		...
		{
			if(currGame.getRound() == 1) // set all players' chips to 100 initially
			{
				threads.setChipTotal(100);
			}
	...
	public void getPlayerBet(int playerIndex, PlayerThread currPlayer, Game currGame)
	{
		...
		s = currPlayer.getPlayerName() + ", it is your turn to make a bet. Your money is " + currPlayer.getChipTotal() + " dollars\nEnter money you want to bet!";
		m = new Message(s);
	...
	public void receivePlayerBet(int playerIndex, PlayerThread currPlayer, Game currGame)
	{
		...
		String s = currPlayer.getPlayerName() + " bet " + currPlayer.getBetAmount() + " dollars";
		Message m = new Message(s);
		broadcastToOthers(m, currPlayer, currGame);
		s = "You bet " + currPlayer.getBetAmount() + " dollars";
		m = new Message(s);
		broadcastToPlayer(m, currPlayer, currGame);
	...
	{		
		...
		if(choice.equals("1") || choice.equals("stand"))
		{
			// Send message to all players with player's choice
			String s = "You standed.";
			Message m = new Message(s);
			broadcastToPlayer(m, currPlayer, currGame);
			s = currPlayer.getPlayerName() + " standed.";
			m = new Message(s);
			broadcastToOthers(m, currPlayer, currGame);
		...
		else if(choice.equals("2") || choice.equals("hit"))
		{
			...
			Message m = new Message(s);
			String ps = "";
			ps += currGame.getPlayerState(currPlayer.getPlayerName(), currPlayer);
			Message a = new Message(ps);
			...
			{
				s = "You busted! You lose " + currPlayer.getBetAmount() + " dollars";
				m = new Message(s);
				broadcastToPlayer(m, currPlayer, currGame);
				s = currPlayer.getPlayerName() + " busted! They lose " + currPlayer.getBetAmount() + " dollars";
				m = new Message(s);
				broadcastToOthers(m, currPlayer, currGame);
	...
	public void dealerAddsCards(Game currGame)
	{
		...
		while(currGame.getDealerCardSum() < 17) // hit until dealer's sum is >= 17
		{
			...
			{
				if(currGame.getDealerCardSum() + 10 >= 17) // if dealer�셲 total + 11 >= 17, value Ace at 11 and �쁲tay��
				{
					currGame.setDealerCardSum(currGame.getDealerCardSum() + 10); // set dealer's sum to Ace w/ value 11
					break;
				}
				// else if dealer�셲 total + 11 > 21 (�쁞ust��), value Ace at 1
			}
			Card c = currGame.getGameDeck().deal();
		...
		String ds = "----------------------------------------------------------------------------\n";
		ds += "DEALER\n\n";
		ds += "Score: " + currGame.getDealerStatus();
		ds += "\nCards: | ";
	...
	{		
		...
		if(currPlayer.busted())
		{
			...
			sToPlayer = "You busted. " + currPlayer.getBetAmount() + " dollars were deducted from your total";
			sToOthers = currPlayer.getPlayerName() + " busted. " + currPlayer.getBetAmount() + " dollars were deducted from " + currPlayer.getPlayerName() + "'s total";
		}
		...
		else if(currPlayer.blackJack() && !currGame.isDealerBlackJack())
		{
			...
			sToPlayer = "You had blackjack. " + (2*currPlayer.getBetAmount()) + " dollars were added to your total";
			sToOthers = currPlayer.getPlayerName() + " had blackjack. " + (2*currPlayer.getBetAmount()) + " dollars were added to " + currPlayer.getPlayerName() + "'s total";
		}
		...
		else if(currPlayer.blackJack() && currGame.isDealerBlackJack())
		{
			...
			sToOthers = currPlayer.getPlayerName() + " had blackjack and so did the dealer. " + currPlayer.getPlayerName() + "'s dollars total remains the same";
		}
		...
		else if(currPlayer.getCardSum() > currGame.getDealerCardSum())
		{
			...
			sToPlayer = "You had a sum greater than the dealer's. " + currPlayer.getBetAmount() + " dollars were added to your total";
			sToOthers = currPlayer.getPlayerName() + " had a sum greater than the dealer's. " + currPlayer.getBetAmount() + " dollars were added to " + currPlayer.getPlayerName() + "'s total";
		}
		...
		else if(currGame.isDealerBusted())
		{
			...
			sToPlayer = "You didn't bust but dealer did. " + currPlayer.getBetAmount() + " dollars were added to your total";
			sToOthers = currPlayer.getPlayerName() + " didn't bust but dealer did. " + currPlayer.getBetAmount() + " dollars were added to " + currPlayer.getPlayerName() + "'s total";
		}
		...
		else if(currPlayer.getCardSum() < currGame.getDealerCardSum())
		{
			...
			sToPlayer = "You had a sum less than the dealer's. " + currPlayer.getBetAmount() + " dollars were deducted from your total";
			sToOthers = currPlayer.getPlayerName() + " had a sum less than the dealer's. " + currPlayer.getBetAmount() + " dollars were deducted from " + currPlayer.getPlayerName() + "'s total";
		}
		// If player's card sum = dealer's card sum
		else if(currPlayer.getCardSum() == currGame.getDealerCardSum())
		{
			...
			sToPlayer = "You had a sum equal to the dealer's. Your dollars total remains the same";
			sToOthers = currPlayer.getPlayerName() + " had a sum equal to the dealer's. Their dollars total remains the same";
		}
		else
		{
	...
	public void endGame(Game currGame, PlayerThread loser)
	{
		...
		{
			s += "\nThe winner of this game is " + winner.getPlayerName() + " with " + winner.getChipTotal() + " dollars.";
		}
		s += "\nThe loser of this game is " + loserName + " with " + loser.getChipTotal() + " dollars.";
		Message m = new Message(s);
		broadcastToAll(m, currGame);
